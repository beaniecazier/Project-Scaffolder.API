...
/// <summary>
/// The collection of Endpoints for the ModelMetaInfo Model in API
/// </summary>
[ApiVersion(1.0)]
public class GetModelMetaInfoModelEndpoint : IEndpoints
{
    private const string ContentType = "application/json";
    private const string ModelTypeName = "ModelMetaInfo";
    
    /// <summary>
    /// Register specific services need for the ModelMetaInfo Model GET endpoints to the DI container
    /// </summary>
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        var singleEndpoint = app.MapGet(
                ModelMetaInfoModelEndpoints.GetByIdEndpoint,
                GetModelMetaInfoModelByIdAsync)
            .WithName("GetModelMetaInfoModelByID")
            ...
            .HasApiVersion(1.0)
            .CacheOutput(ModelMetaInfoModelEndpoints.Tag)
            .WithTags(ModelMetaInfoModelEndpoints.Tag);

        var multipleEndpoint = app.MapGet(
                ModelMetaInfoModelEndpoints.GetAllEndpoint,
                GetAllModelMetaInfoModelsAsync)
            .WithName("GetAllModelMetaInfoModels")
    ...
    private static async Task<IResult> GetModelMetaInfoModelByIdAsync(
        HttpContext http,
        string id,
    ...
    /// <summary>
    /// Retrieve all ModelMetaInfo Models from the database
    /// </summary>
    /// <param name="http">The http client context</param>
    /// <param name="service">The service class the serves this Endpoint for database operations</param>
    /// <param name="request">The encapsulated GetAllModels Request Parameters</param>
    /// <param name="token">Cancellation token</param>
    /// <returns>A list of all ModelMetaInfo Models in the database</returns>
    /// <response code="200">Get all successful</response>
    /// <response code="500">Something went wrong or the database does not exist</response>
    private static async Task<IResult> GetAllModelMetaInfoModelsAsync(
        HttpContext http,
        IModelMetaInfoModelService service,
    ...
    private static IResult HandleGetAllModelsFromRequestEndpointSuccess(
        IEnumerable<ModelMetaInfoModel> models,
        GetAllModelsOptions options,
        int total,
        string userId)
    {
        var responsesCollection = new ModelMetaInfoModelsResponse()
        {
