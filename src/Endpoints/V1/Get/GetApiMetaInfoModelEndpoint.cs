...
/// <summary>
/// The collection of Endpoints for the ApiMetaInfo Model in API
/// </summary>
[ApiVersion(1.0)]
public class GetApiMetaInfoModelEndpoint : IEndpoints
{
    private const string ContentType = "application/json";
    private const string ModelTypeName = "ApiMetaInfo";
    
    /// <summary>
    /// Register specific services need for the ApiMetaInfo Model GET endpoints to the DI container
    /// </summary>
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        var singleEndpoint = app.MapGet(
                ApiMetaInfoModelEndpoints.GetByIdEndpoint,
                GetApiMetaInfoModelByIdAsync)
            .WithName("GetApiMetaInfoModelByID")
        ...
        var multipleEndpoint = app.MapGet(
                ApiMetaInfoModelEndpoints.GetAllEndpoint,
                GetAllApiMetaInfoModelsAsync)
            .WithName("GetAllApiMetaInfoModels")
    ...
    private static async Task<IResult> GetApiMetaInfoModelByIdAsync(
        HttpContext http,
        string id,
    ...
    /// <summary>
    /// Retrieve all ApiMetaInfo Models from the database
    /// </summary>
    /// <param name="http">The http client context</param>
    /// <param name="service">The service class the serves this Endpoint for database operations</param>
    /// <param name="request">The encapsulated GetAllModels Request Parameters</param>
    /// <param name="token">Cancellation token</param>
    /// <returns>A list of all ApiMetaInfo Models in the database</returns>
    /// <response code="200">Get all successful</response>
    /// <response code="500">Something went wrong or the database does not exist</response>
    private static async Task<IResult> GetAllApiMetaInfoModelsAsync(
        HttpContext http,
        IApiMetaInfoModelService service,
    ...
    private static IResult HandleGetAllModelsFromRequestEndpointSuccess(
        IEnumerable<ApiMetaInfoModel> models,
        GetAllModelsOptions options,
        int total,
        string userId)
    {
        var responsesCollection = new ApiMetaInfoModelsResponse()
        {
